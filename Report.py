...
def DbConnect():
    cursor = sqlite3.connect("./Reactor.db")
    return cursor
...
def GetReactorData(cursor):
     res = cursor.execute("SELECT * FROM Reactor")
     for row in res.fetchall():
         print('{0} : {1}, {2}, {3}, {4}, {5}, {6}, {7}, {8}'.format(row[0], row[1], row[2], row[3], row[4], row[5], row[6], row[7], row[8]))

def GetReactorUsernamesStatistic(cursor):
     res = cursor.execute("SELECT COUNT(DISTINCT UPPER(user)) FROM Reactor")
     for row in res.fetchall():
         print(color('\n[+] In total {0} unique user accounts were captured.'.format(row[0]), code = 2, modifier = 1))

def GetReactorUsernames(cursor):
     res = cursor.execute("SELECT DISTINCT user FROM Reactor")
     for row in res.fetchall():
         print('User account: {0}'.format(row[0]))

def GetReactorUsernamesWithDetails(cursor):
     res = cursor.execute("SELECT client, user, module, type, cleartext FROM Reactor WHERE UPPER(user) in (SELECT DISTINCT UPPER(user) FROM Reactor) ORDER BY client")
     for row in res.fetchall():
         print('IP: {0} module: {1}:{3}\nuser account: {2}'.format(row[0], row[2], row[1], row[3]))


def GetReactorCompleteHash(cursor):
     res = cursor.execute("SELECT fullhash FROM Reactor WHERE UPPER(user) in (SELECT DISTINCT UPPER(user) FROM Reactor)")
     for row in res.fetchall():
...
GetReactorUsernames(cursor)
print(color("\n[+] Username details:", code = 2, modifier = 1))
GetReactorUsernamesWithDetails(cursor)
GetReactorUsernamesStatistic(cursor)
