...
config.read(os.path.join(BASEDIR,'Reactor.conf'))
RespondTo           = [_f for _f in [x.upper().strip() for x in config.get('Reactor Core', 'RespondTo').strip().split(',')] if _f]
DontRespondTo       = [_f for _f in [x.upper().strip() for x in config.get('Reactor Core', 'DontRespondTo').strip().split(',')] if _f]
Interface           = settings.Config.Interface
Reactor_IP        = RespondWithIP()
ROUTERIP            = Reactor_IP # Set to Reactor_IP in case we fall on a static IP network and we don't get a DHCP Offer. This var will be updated with the real dhcp IP if present.
NETMASK             = "255.255.255.0"
DNSIP               = "0.0.0.0"
DNSIP2              = "0.0.0.0"
DNSNAME             = "local"
WPADSRV             = "http://"+Reactor_IP+"/wpad.dat"
