...
parser.add_option('-6', "--externalip6",    action="store",      help="Poison all requests with another IPv6 address than Reactor's one.", dest="ExternalIP6",  metavar="2002:c0a8:f7:1:3ba8:aceb:b1a9:81ed", default=None)
parser.add_option('-e', "--externalip",    action="store",      help="Poison all requests with another IP address than Reactor's one.", dest="ExternalIP",  metavar="10.0.0.22", default=None)
...
parser.add_option('-Q','--quiet',           action="store_true", help="Tell Reactor to be quiet, disables a bunch of printing from the poisoners. Default: False", dest="Quiet", default=False)
...
if not os.geteuid() == 0:
    print(color("[!] Reactor must be run as root."))
    sys.exit(-1)
...
#Create the DB, before we start Reactor.
CreateReactorDb()
...
def serve_thread_SSL(host, port, handler):
	try:
		...
		cert = os.path.join(settings.Config.ReactorPATH, settings.Config.SSLCert)
		key =  os.path.join(settings.Config.ReactorPATH, settings.Config.SSLKey)
...
def main():
	try:
		...
		#// Vintage Reactor BOWSER module, now disabled by default. 
		...
		if settings.Config.AnalyzeMode:
			print(color('[+] Reactor is in analyze mode. No NBT-NS, LLMNR, MDNS requests will be poisoned.', 3, 1))
		if settings.Config.Quiet_Mode:
			print(color('[+] Reactor is in quiet mode. No NBT-NS, LLMNR, MDNS messages will print to screen.', 3, 1))
