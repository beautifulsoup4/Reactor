...
def ShowWelcome():
    print(color('\nReactor MultiRelay %s NTLMv1/2 Relay' %(__version__),8,1))
    print('\nSend bugs/hugs/comments to: laurent.gaffie@gmail.com')
    print('Usernames to relay (-u) are case sensitive.')
    print('To kill this script hit CTRL-C.\n')
    print(color('/*',8,1))
    print('Use this script in combination with Reactor.py for best results.')
    print('Make sure to set SMB and HTTP to OFF in Reactor.conf.\n')
    print('This tool listen on TCP port 80, 3128 and 445.')
    print('For optimal pwnage, launch Reactor only with these 2 options:')
...
def RunShellCmd(data, s, clientIP, Target, Username, Domain):
    ...
    if data[8:10] == b'\x73\x6d':
        ...
        print("[+] Hashes were saved anyways in Reactor/logs/ folder.\n")
    ...
    if data[8:10] == b'\x75\x00' and OneCommand == None:
        if Pivoting[0] == "1":
            ...
            print("[+] Authenticated.\n[+] Dropping into Reactor's interactive shell, type \"exit\" to terminate\n")
            ShowHelp()
